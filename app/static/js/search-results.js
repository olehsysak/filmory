...
// Pagination and loaded counters for each search mode (films / persons / mixed)
let filmsPage = 1;
let personsPage = 1;
let mixedPage = 1;
let filmsLoaded = 0;
...
mixedMoreBtn.addEventListener('click', async function () {
    try {
        ...
        const list = mixedGrid.querySelector('.search-mixed-list');
        list.innerHTML += items.map(renderMixedItem).join('');
        mixedLoaded += items.length;
        mixedCount.textContent = mixedLoaded + ' shown';
...
filmsMoreBtn.addEventListener('click', async function () {
    ...
    try {
        ...
        const list = filmsGrid.querySelector('.search-film-list');
        list.innerHTML += films.map(renderFilmRow).join('');
        filmsLoaded += films.length;
        filmsCount.textContent = filmsLoaded + ' shown';
...
personsMoreBtn.addEventListener('click', async function () {
    ...
    try {
        ...
        const list = personsGrid.querySelector('.search-person-list');
        list.innerHTML += persons.map(renderPersonRow).join('');
        personsLoaded += persons.length;
        personsCount.textContent = personsLoaded + ' shown';
...
// Initialize search category tabs (All / Films / People)
function initTabs() {
...
// Initialize search page if query exists
if (SEARCH_QUERY) {
    loadMixed();
    initTabs();
}